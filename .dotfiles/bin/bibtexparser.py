...
class BibTexParser(object):
    ...
    def string_subst(self, val):
        ...
        if '\\' in val or '{' in val:
            ...
            # protect accents like \'e into \'{e}
            val = re.sub(r"(\\['\"^`=~])\\?(\w)", lambda m: m.group(1) + "{" + m.group(2) + "}", val)

            # remove \cite{}
            val = re.sub(r"~\\cite{.*}", lambda m: "", val)

            val = re.sub(r"{(\\[^i]\w*)}", lambda m: m.group(1) + " ", val)
            ...
            # def remove_math(m):
                # return m.group(1) + " "
            # val = re.sub(r"\$(.*)\$", remove_math, val)

            for c in self.unicode_to_latex:
                utf8 = c[0]
                latex = c[1]
                # before = val
                val = val.replace(latex, utf8)
                # after = val
                # if before != after:
                    # debug("before: "+before)
                    # debug("after:  "+after)

            val = re.sub(r"\\\w+{(.*)}", lambda m: m.group(1), val)
            # before = val
    ...
    def getnames(self, names):
        """
        Make people names as surname, firstnames or surname, initials. should
        eventually combine up the two.
        """
        tidynames = []

...
class MediaWiki(object):
    ...
    def call2(self, **params):
        """
        Make an API call to the wiki. *params* is a dictionary of query string
        arguments. For example, to get basic information about the wiki, run:

        >>> wiki.call2(action='query', meta='siteinfo')
    ...
    def get_edittoken(self, title):
        r = self.call2(action="query", prop="info|revisions", intoken="edit", titles=title)
        for page in r['query']['pages'].values():
            if 'edittoken' in page:
                return page['edittoken']

    def get_deletetoken(self, title):
        r = self.call2(action="query", prop="info", intoken="delete", titles=title)
        for page in r['query']['pages'].values():
            if 'deletetoken' in page:
                return page['deletetoken']

    def get_page(self, title):
        r = self.call2(action="query", titles=title, prop="revisions", rvprop="content")
        try:
            for page in r['query']['pages'].values():
    ...
    def update_page(self, title, content):
        current_content = unicodedata.normalize('NFKD', self.get_page(title))
        new_content = unicodedata.normalize('NFKD', content)
        if current_content != new_content:
            edittoken = self.get_edittoken(title)
            return self.call2(action="edit", title=title, text=content, bot=True, token=edittoken)
